...
public interface TeamNoticeReadRepository extends JpaRepository<TeamNoticeRead, Long> {
    ...
    @Query(
            "SELECT r.teamNotice.teamNoticeId FROM TeamNoticeRead r WHERE r.teamNotice IN :notices AND r.createdBy = :userId")
    ...
    @Modifying
    @Query("DELETE FROM TeamNoticeRead r WHERE r.teamNotice.createdBy = :createdBy")
    void deleteByTeamNoticeCreatedBy(@Param("createdBy") Long createdBy);

    @Modifying
    @Query("DELETE FROM TeamNoticeRead r WHERE r.createdBy = :createdBy")
