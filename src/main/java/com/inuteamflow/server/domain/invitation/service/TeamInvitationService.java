...
public class TeamInvitationService {
    ...
    public Page<TeamInvitationResponse> getInvitations(User user, InvitationDirection direction, Pageable pageable) {
        if (direction == InvitationDirection.RECEIVED) {
            return teamInvitationRepository.findByReceiver(user, pageable).map(invitation -> {
                String senderName = invitation.getCreatedBy() == null
                        ? null
                        : userRepository
                                .findById(invitation.getCreatedBy())
                                .map(User::getName)
                                .orElse(null);
            ...
            return teamInvitationRepository
                    .findByCreatedBy(user.getUserId(), pageable)
    ...
    public TeamInvitationResponse invite(User sender, Long teamId, TeamInvitationCreateRequest request) {
        ...
        TeamMember senderMember = teamMemberRepository
                .findByTeamAndUser(team, sender)
        ...
        User receiver = userRepository
                .findByStudentNumber(request.getStudentNumber())
        ...
        TeamInvitation invitation = teamInvitationRepository
                .findByTeam_TeamIdAndReceiver(teamId, receiver)
                .map(existing -> {
                ...
                .orElseGet(() -> teamInvitationRepository.save(TeamInvitation.create(team, receiver)));
     ...
     */
    public List<InvitationCandidateResponse> getCandidates(User user, Long teamId, String name) {
        ...
        Set<Long> memberIds =
                members.stream().map(TeamMember::getUser).map(User::getUserId).collect(Collectors.toSet());
        ...
        return candidates.stream()
                .map(candidate -> InvitationCandidateResponse.from(
                        candidate, resolveCandidateStatus(candidate.getUserId(), memberIds, pendingIds)))
                .toList();
    }
    ...
    public TeamInvitationResponse updateStatus(
        ...
        if (newStatus == Status.ACCEPTED) {
            invitation.accept();
            teamMemberRepository.save(TeamMember.create(invitation.getTeam(), receiver, TeamRole.MEMBER));
    ...
    private InvitationCandidateStatus resolveCandidateStatus(
            Long candidateId, Set<Long> memberIds, Set<Long> pendingInvitationReceiverIds) {
