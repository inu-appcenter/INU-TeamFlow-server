...
public interface InfoPostRepository extends JpaRepository<InfoPost, Long> {
    ...
    @Query(
            value = "SELECT p FROM InfoPost p " + "WHERE (:categories IS NULL OR p.category IN :categories) "
                    + "AND (:keyword IS NULL OR p.title LIKE %:keyword%)",
            countQuery =
                    "SELECT COUNT(p) FROM InfoPost p " + "WHERE (:categories IS NULL OR p.category IN :categories) "
                            + "AND (:keyword IS NULL OR p.title LIKE %:keyword%)")
    Page<InfoPost> search(
            @Param("categories") List<InfoPostCategory> categories,
