...
public interface RecurrenceExceptionParticipantRepository extends JpaRepository<RecurrenceExceptionParticipant, Long> {
    ...
    @Query("""
    SELECT rep FROM RecurrenceExceptionParticipant rep
    JOIN FETCH rep.teamMember tm
    JOIN FETCH tm.user
    WHERE rep.recurrenceException IN :recurrenceExceptions
    """)
    List<RecurrenceExceptionParticipant> findByRecurrenceExceptionInWithMember(
            @Param("recurrenceExceptions") List<RecurrenceException> recurrenceExceptions);

    // THIS_INSTANCE sync 시 기존 참착자 교체를 위한 삭제
    void deleteByRecurrenceException(RecurrenceException recurrenceException);
    ...
    void deleteByEventAndOriginalOccurrenceAtGreaterThanEqual(
            @Param("event") Event event, @Param("occurrenceAt") LocalDateTime occurrenceAt);
    ...
    AND p.recurrenceException.originalOccurrenceAt = :occurrenceAt
    """)
    void deleteByEventAndOccurrenceAt(@Param("event") Event event, @Param("occurrenceAt") LocalDateTime occurrenceAt);
